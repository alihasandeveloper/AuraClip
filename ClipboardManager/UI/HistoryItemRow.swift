...
struct HistoryItemRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 10) {
            // Clickable Content Body (Thumbnail + Info)
            HStack(alignment: .center, spacing: 10) {
            ...
            // Trailing Action Glass Buttons on Hover or Selection
            if isHovered || isSelected {
                HStack(spacing: 4) {
                    // Open URL in Default Browser Button
                    if let url = item.openableURL {
                        Button(action: {
                            onOpenURL?(url)
                        }) {
                            HStack(spacing: 4) {
                            ...
                            .background(
                                Capsule()
                                    .fill(isSelected ? Color.white.opacity(0.26) : Color.blue.opacity(0.12))
                            )
                            .overlay(
                                Capsule()
                                    .stroke(
                                        isSelected ? Color.white.opacity(0.42) : Color.blue.opacity(0.3),
                                        lineWidth: 0.8
                                    )
                            )
                            .foregroundColor(isSelected ? .white : Color(red: 0.05, green: 0.48, blue: 0.98))
                        }
                        .buttonStyle(.plain)
